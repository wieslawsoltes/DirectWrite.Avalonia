...
namespace Avalonia.Direct2D1.Media
{
    ...
    internal unsafe class DrawingContextImpl : IDrawingContextImpl
    {
        ...
        public void DrawRectangle(IBrush? brush, IPen? pen, RoundedRect rrect, BoxShadows boxShadow = default)
        {
            ...
            var isRounded = Math.Abs(radiusX) > 0.0001 || Math.Abs(radiusY) > 0.0001;
        ...
        public BrushImpl CreateBrush(IBrush? brush, Rect destinationRect)
        {
            ...
            else if (imageBrush?.Source is Avalonia.Media.Imaging.Bitmap bitmapSource)
            {
                return new ImageBrushImpl(
                    imageBrush,
                    _deviceContext,
                    GetBitmapImpl(bitmapSource),
                    destinationRect);
            }
